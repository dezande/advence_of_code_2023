require 'minitest/autorun'
require_relative '../app/scratchcards'

class ScratchcardsTest < Minitest::Test
  def test_example
    lines = "Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53\n"
    lines += "Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19\n" 
    lines += "Card 3:  1 21 53 59 44 | 69 82 63 72 16 21 14  1\n" 
    lines += "Card 4: 41 92 73 84 69 | 59 84 76 51 58  5 54 83\n"
    lines += "Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36\n"
    lines += 'Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11'
    scratchcards = Scratchcards.new(lines)
    assert_equal 13, scratchcards.score_sum
    assert_equal 30, scratchcards.nb_copy
  end
end